...
function createMarkersThread()
	for market_id,market_data in pairs(Config.market_locations) do
		...
		Citizen.CreateThreadNow(function()
			...
			-- Only create owner marker if coordinates are not 0,0,0 and store can be owned
			if not (x == 0 and y == 0 and z == 0) then
				local market_type = Config.market_types[market_data.type]
				if market_type and market_type.max_purchasable_categories > 0 then
					while true do
						local timer = 1500
...
function createTargetsThread()
	Citizen.CreateThreadNow(function()
		for market_id,market_data in pairs(Config.market_locations) do
			...
			-- Only create owner target if coordinates are not 0,0,0 and store can be owned
			if not (x == 0 and y == 0 and z == 0) then
				local market_type = Config.market_types[market_data.type]
				if market_type and market_type.max_purchasable_categories > 0 then
			...
			for customer_blip_id,customer_blip_location in pairs(market_data.sell_blip_coords) do
				local x,y,z = table.unpack(customer_blip_location)
				-- Only create customer target if coordinates are valid
