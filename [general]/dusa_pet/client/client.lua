            ...
            if DoesEntityExist(pet) then
                ...
                elseif dst < 4 then
                    ...
                    if returnBall then
                        ...
                        DeleteEntity(ballObj)
                        TriggerServerEvent('dusa-pets:returnBall', 'add')
                        returnBall = false
                    end
                end
...
AddEventHandler('dusa-pets:cl:useBall', function()
    dusa.serverCallback('dusa-pets:cb:checkItem', function(check)
        if check then
            ...
            ballThrown = true
            chasing = true
            TriggerServerEvent('dusa-pets:returnBall', 'remove')        
        else
...
RegisterNUICallback('itemData', function(data)
    local total = lib.callback.await('dusa_pets:cb:canBuyItem', false, data)
    if #data.data == 0 then
