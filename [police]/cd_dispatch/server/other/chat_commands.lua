    ...
    AddEventHandler('cd_dispatch:CallCommand', function(data, data_2)
        ...
        if Config.AntiCheat.ENABLE then
            ...
            if Config.AntiCheat.BannedWords.ENABLE then
                if ContainsBannedWords(_source, 'cd_dispatch:CallCommand', {data_2.title, data_2.message, data_2.char_name, data_2.phone_number, data_2.job_label, data.street, data.sex}) then return end
            end
        end
