...
local function CleanupRemovedPatreonVehicles()
    ...
    local allPatreonVehicles = DatabaseQuery('SELECT DISTINCT vehicle FROM '..vehicleTable..' WHERE ' .. garageCondition)
    ...
    if allPatreonVehicles and allPatreonVehicles[1] then
        ...
        if #vehiclesToRemove > 0 then
            for i = 1, #vehiclesToRemove do
                ...
                local plates = DatabaseQuery('SELECT plate FROM '..vehicleTable..' WHERE vehicle = "'..veh..'" AND (' .. garageCondition .. ')')
                ...
                if plates and plates[1] then
                    ...
                    DatabaseQuery('DELETE FROM '..vehicleTable..' WHERE plate IN ('..inPlates..')')
                    
                    -- Remove from garage keys if table exists
...
local function CreateUniquePlate()
    ...
    while tries < 50 do
        ...
        local r = DatabaseQuery('SELECT plate FROM '..vehicleTable..' WHERE plate="'..p..'"')
...
EnsurePatreonVehiclesForPlayer = function(src, Player, tier)
    ...
    local allSet = {}
    local vehicleTable = Config.FrameworkSQLtables.vehicle_table
    local vehicleProps = Config.FrameworkSQLtables.vehicle_props
    local vehicleIdentifier = Config.FrameworkSQLtables.vehicle_identifier
    ...
    if #toGrant == 0 then
        ...
        for i = 1, #toGrant do
            ...
            local check = DatabaseQuery('SELECT plate FROM '..vehicleTable..' WHERE '..vehicleIdentifier..'="'..citizenid..'" AND vehicle="'..spawn..'" LIMIT 1')
            if not check or not check[1] then
                ...
                DatabaseQuery('INSERT INTO '..vehicleTable..' ('..vehicleIdentifier..', plate, '..vehicleProps..', garage_type, in_garage, license, hash, vehicle, garage_id) VALUES (@'..vehicleIdentifier..', @plate, @'..vehicleProps..', @garage_type, @in_garage, @license, @hash, @vehicle, @garage_id)',
                {
                    ['@'..vehicleIdentifier..''] = citizenid,
                    ['@plate'] = plate,
                    ['@'..vehicleProps..''] = props,
    ...
    if #allList > 0 then
        ...
        local owned = DatabaseQuery('SELECT plate, vehicle FROM '..vehicleTable..' WHERE '..vehicleIdentifier..'="'..citizenid..'" AND vehicle IN ('..inList..')')
        ...
        if #platesToRemove > 0 then
            ...
            DatabaseQuery('DELETE FROM '..vehicleTable..' WHERE '..vehicleIdentifier..'="'..citizenid..'" AND plate IN ('..inPlate..')')
            
            -- Remove from garage keys table if enabled
            if Config.VehicleKeys.ENABLE then
...
RegisterCommand('anchorpatreon', function(src, args)
    ...
    for i = 1, #spawnList do
        ...
        local q = 'UPDATE '..vehicleTable..' SET in_garage=1, garage_id="'..garageId..'", garage_type="'..vehicleData.type..'" WHERE '..Config.FrameworkSQLtables.vehicle_identifier..'="'..citizenid..'" AND vehicle="'..vehicleData.vehicle..'"'
        DatabaseQuery(q)
...
exports('IsVehicleAllowedByTier', IsVehicleAllowedByTier)
exports('IsVehicleInAnyTier', IsVehicleInAnyTier)
exports('GetPlayerPatreonTier', GetPlayerPatreonTier)
