    ...
    if not IsProgressBarActive() then
        if animation.emote.enabled then
            ...
            if animation.emote.prop.model then
                ...
                local boneIndex = GetPedBoneIndex(cache.ped, animation.emote.prop.bone)
                AttachEntityToEntity(prop, cache.ped, boneIndex, animation.emote.prop.pos.x,animation.emote.prop.pos.y,animation.emote.prop.pos.z, animation.emote.prop.rot.x, animation.emote.prop.rot.y, animation.emote.prop.rot.z, true, true, false, false, 1, true)
            end
            if Config.Target == 'ox_target' then
