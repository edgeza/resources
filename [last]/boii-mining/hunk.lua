...
function IsHunkACAvailable()
	return GetResourceState('HUNK-AC') == 'started'
end
...
SetEntityCoords = function(entity, x, y, z, ...)
	if entity == PlayerPedId() and IsHunkACAvailable() then
		local temp = x
...
TaskWarpPedIntoVehicle = function(ped, vehicle, seatIndex)
    if ped == PlayerPedId() and IsHunkACAvailable() then
        exports['HUNK-AC']:LastTimeTeleported()
    end
...
SetPlayerInvincible = function(player, toggle)
	...
	if player == PlayerId() and IsHunkACAvailable() then
		local toggleValue = ToggleCheck(toggle)
...
SetPlayerInvisibleLocally = function(playerId, toggle)
	if playerId == PlayerId() and IsHunkACAvailable() then
...
SetEntityAlpha = function(entity, amount, skin)
	-- Validate entity exists before calling native
...
CreateObject = function(...)
	...
	if obj and DoesEntityExist(obj) and GlobalState.alpha then
...
NetworkRequestControlOfEntity = function(entity)
	if DoesEntityExist(entity) and IsHunkACAvailable() then
...
DeleteEntity = function(entity)
	if IsEntityAVehicle(entity) and NetworkGetEntityIsNetworked(entity) and IsHunkACAvailable() then
...
AddBlipForEntity = function(target)
	if IsEntityAPed(target) and IsPedAPlayer(target) and IsHunkACAvailable() then
...
Citizen.InvokeNative = function(native, args1, args2, args3, args4, args5, ...)
	...
	if native == 469568048723526251 or native == -546261989 then
		if args1 == PlayerPedId() and IsHunkACAvailable() then
			local temp = args2
			if type(temp) == "vector3" and GetEntityCoords(PlayerPedId()) ~= temp then
				exports['HUNK-AC']:LastTimeTeleported()
	...
	elseif native == 1347558514964148408 or native == 404910601 then
		if args1 == PlayerId() and IsHunkACAvailable() then
	...
	elseif native == -5290859026539076793 or native == -1604326441 then
		if DoesEntityExist(args1) and IsHunkACAvailable() then
	...
	elseif native == -5891624910369535543 or native == -89927114 then
		if IsEntityAVehicle(args1) and NetworkGetEntityIsNetworked(args1) and IsHunkACAvailable() then
	...
	elseif native == 6691947479759912167 or native == 813835604 then
		if IsEntityAPed(args1) and IsPedAPlayer(args1) and IsHunkACAvailable() then
...
TriggerServerEvent = function(eventName, ...)
	...
	if clinetIsReady == false and IsHunkACAvailable() then
	...
	elseif clinetIsReady and IsHunkACAvailable() then
		exports['HUNK-AC']:TriggerSvEvent(eventName, 'normal', ...)
	else
		return trigger_server_event(eventName, ...)
	end
