...
local function DebugPrint(msg)
    if DEBUG_MODE then
        print("^3[OLRP-bossmenu Debug]^7 " .. msg)
        LAST_DEBUG = msg
    end
end
...
RegisterNetEvent('olrp-bossmenu:client:JobChanged', function(jobName)
    if menuOpen and PlayerData.job and PlayerData.job.isboss and PlayerData.job.name == jobName then
        QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetJobData', function(jobData)
            if jobData then
                SendNUIMessage({
                    action = "refreshData",
                    jobData = jobData
        ...
        if Config.TargetSystem == "qb-target" then
            exports['qb-target']:AddBoxZone("job_application_"..jobName, location.coords, location.length, location.width, {
                ...
                options = {
                    {
                        type = "client",
                        event = "olrp-bossmenu:client:OpenApplicationForm",
                ...
                options = {
                    {
                        ...
                        label = location.label,
                        onSelect = function()
                            TriggerEvent("olrp-bossmenu:client:OpenApplicationForm", {jobData = jobName})
                        end,
                        distance = 2.0
                    }
                }
...
RegisterNetEvent('olrp-bossmenu:client:OpenApplicationForm', function(data)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:CheckPendingApplication', function(hasPendingApplication)
...
RegisterNetEvent('olrp-bossmenu:client:SyncPermissions', function(targetCitizenid, permissions)
...
RegisterNUICallback('getApplications', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetJobApplications', function(applications)
        cb(applications)
    end, jobName)
end)

RegisterNUICallback('updateApplicationStatus', function(data, cb)
    TriggerServerEvent('olrp-bossmenu:server:UpdateApplicationStatus', data.applicationId, data.status, data.notes)
    cb('ok')
end)
            ...
            if Config.TargetSystem == "qb-target" then
                exports['qb-target']:AddBoxZone(zoneName, location.coords, location.length, location.width, {
                    ...
                    options = {
                        {
                            type = "client",
                            event = "olrp-bossmenu:client:TriggerOpenManager", 
                    ...
                    options = {
                        {
                            ...
                            label = "Manage " .. jobLabel,
                            onSelect = function()
                                TriggerEvent("olrp-bossmenu:client:TriggerOpenManager", {jobData = jobName})
                            end,
                            canInteract = function()
                                if PlayerData.job and PlayerData.job.name == jobName then
                                    if PlayerData.job.isboss then
...
RegisterNetEvent('olrp-bossmenu:client:TriggerOpenManager', function(data)
    TriggerServerEvent('olrp-bossmenu:server:RequestRefreshJobData')
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:HasJobAccess', function(hasAccess) 
...
RegisterNUICallback('updateEmployeePermissions', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:UpdateEmployeePermissions', function(result)
        cb(result) 
...
RegisterNUICallback('getEmployeePermissions', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetEmployeePermissions', function(permissions)
        cb(permissions)
    end, citizenid, jobName)
end)

RegisterNetEvent('olrp-bossmenu:client:RefreshPermissions', function(permissions)
    ...
    if permissions then
        ...
        QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetEmployeePermissions', function(newPermissions)
...
function OpenJobManager(jobName)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetJobData', function(jobData)
        ...
        QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetSettings', function(settings)
            if not settings then
                return
            end
            QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetSocietyData', function(societyData)
...
RegisterNetEvent('olrp-bossmenu:client:OpenManager', function()
    DebugPrint("Blocked automatic opening from original event")
end)

-- Refresh data event handler
RegisterNetEvent('olrp-bossmenu:client:RefreshData', function()
    ...
    local menuOptions = {
        {
            ...
            params = {
                event = "olrp-bossmenu:client:FireEmployeeConfirmed",
                args = {
                    citizenid = citizenid,
                    confirmed = true
                }
...
RegisterNUICallback("submitApplication", function(data, cb) 
    SetNuiFocus(false, false)
    TriggerServerEvent("olrp-bossmenu:server:SubmitApplication", data.jobName, data.answers)
end)
...
RegisterNetEvent('olrp-bossmenu:client:FireEmployeeConfirmed', function(data)
    SendNUIMessage({
        action = "fireEmployeeResponse",
        confirmed = data.confirmed,
        citizenid = data.citizenid
...
RegisterNUICallback('updateEmployee', function(data, cb)
    TriggerServerEvent('olrp-bossmenu:server:UpdateEmployee', data.citizenid, PlayerData.job.name, data.grade)
    cb('ok')
end)

RegisterNUICallback('removeEmployee', function(data, cb)
    TriggerServerEvent('olrp-bossmenu:server:RemoveEmployee', data.citizenid, PlayerData.job.name)
    cb({success = true, message = "Employee removal request sent"})
end)

-- Listen for employee fired event
RegisterNetEvent('olrp-bossmenu:client:EmployeeFired', function(success, message)
    SendNUIMessage({
...
RegisterNUICallback('saveSettings', function(data, cb)
    TriggerServerEvent('olrp-bossmenu:server:SaveSettings', data)
    cb('ok')
end)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if Config.TargetSystem ~= "qb-target" and Config.TargetSystem ~= "ox_target" then
        print("^1[OLRP-bossmenu]^7 Invalid target system in config: " .. (Config.TargetSystem or "nil") .. ". Defaulting to qb-target.")
...
CreateThread(function()
    while true do
        ...
        if menuOpen then
            TriggerServerEvent('olrp-bossmenu:server:RefreshPlayTime')
        end
...
RegisterNUICallback('depositMoney', function(data, cb)
    ...
    TriggerServerEvent('olrp-bossmenu:server:DepositMoney', data.amount, data.note, data.jobName)
    cb({success = true})
end)
...
RegisterNUICallback('withdrawMoney', function(data, cb)
    ...
    TriggerServerEvent('olrp-bossmenu:server:WithdrawMoney', data.amount, data.note, data.jobName)
    cb({success = true})
end)
...
RegisterNUICallback('transferMoney', function(data, cb)
    ...
    TriggerServerEvent('olrp-bossmenu:server:TransferMoney', data.citizenid, data.amount, data.note, data.jobName)
    cb({success = true})
end)
...
RegisterNetEvent('olrp-bossmenu:client:OpenApplicationForm', function(data)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:CheckApplicationStatus', function(result)
...
RegisterNUICallback('getPlaytimeData', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetPlaytimeData', function(playtimeData)
...
RegisterNUICallback('hireEmployee', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:HireNewEmployee', function(result)
        cb(result)
...
RegisterNUICallback('updateJobGrade', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:UpdateJobGrade', function(result)
        cb(result)
...
RegisterNUICallback('addJobGrade', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:AddJobGrade', function(result)
        cb(result)
...
RegisterNUICallback('deleteJobGrade', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:DeleteJobGrade', function(result)
        cb(result)
...
RegisterNUICallback('getJobGrades', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('olrp-bossmenu:server:GetJobGrades', function(gradesData)
        if gradesData then
            cb(gradesData)
        else
            cb(false)
