...
function IsAllowed_Impound()
    local jobName = GetJob().name
    if Config.Impound.Authorized_Jobs[jobName] then
        -- Towing job doesn't need duty, other jobs do
        if jobName == 'towing' then
            return true
        elseif on_duty then
            return true
        end
...
CreateThread(function()
    while not Authorised do Wait(1000) end
    -- Chat suggestion for testing command
    TriggerEvent('chat:addSuggestion', '/setpatreontier', 'Set a player\'s Patreon tier (QB metadata). Usage: /setpatreontier <tier> or /setpatreontier <id> <tier>', {
        { name = 'id_or_tier', help = 'Tier (0-4) if targeting yourself, or Target ID if targeting someone else' },
        { name = 'tier', help = 'Tier number 0-4 (when providing an ID)' }
    })
    ...
    while true do
        ...
        for cd = 1, #Config.Locations do
            ...
            if dist <= self.Dist then
                wait = 5
                inZone = true
                -- Patreon-only garage access gate (when location has PatreonTierRequired)
                local patreonGate = (self.PatreonTierRequired ~= nil and Config.PatreonTiers and Config.PatreonTiers.ENABLE)
                local hasTier = true
                if patreonGate then
                    hasTier = false
                    if Config.Framework == 'qbcore' then
                        local pdata = QBCore.Functions.GetPlayerData()
                        local md = pdata and pdata.metadata or {}
                        local key = Config.PatreonTiers.metadata_key or 'patreon_tier'
                        local t = md[key]
                        local tierNum = 0
                        if type(t) == 'number' then tierNum = t elseif type(t) == 'string' then tierNum = tonumber(t) or 0 end
                        if tierNum >= (self.PatreonTierRequired or 1) then
                            hasTier = true
                        end
                    end
                end
                ...
                if patreonGate and not hasTier then
                    GlobalText = L('patreon_garage_denied')
                end
                if InVehicle() then
                    if patreonGate then
                        GlobalText = '<b>Patreon Garage</b></p>'..L('notif_storevehicle')
                    else
...
function GetFuel(vehicle, plate) --This gets triggered just before you store your vehicle.
    ...
    elseif GetResourceState('rcore_fuel') == 'started' then
        exports["rcore_fuel"]:GetFuel(vehicle)
...
function SetFuel(vehicle, plate, fuel_level) --This gets triggered after you spawn your vehicle.
    if GetResourceState('rcore_fuel') == 'started' then
        exports['rcore_fuel']:SetFuel(vehicle, fuel_level)
...
function Notification(notif_type, message)
    if notif_type and message then
        ...
        elseif Config.Notification == 'other' then
            ...
            if notif_type == 1 then
                exports['brutal_notify']:SendAlert(L('garage'), message, Time, 'success', false)
            elseif notif_type == 2 then
                exports['brutal_notify']:SendAlert(L('garage'), message, Time, 'info', false)
            elseif notif_type == 3 then
                exports['brutal_notify']:SendAlert(L('garage'), message, Time, 'error', false)
            end
