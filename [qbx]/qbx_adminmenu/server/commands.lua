...
lib.addCommand('admincar', {
    ...
    local vehiclesHash = exports.qbx_core:GetVehiclesByHash()
    if not vehiclesHash or not vehiclesHash[vehModel] then
    ...
    if not vehName or not props then
        return exports.qbx_core:Notify(source, 'Failed to get vehicle information. Please try again.', 'error')
    end
