...
RegisterServerEvent('pl_koi:icemachine:addWater')
AddEventHandler('pl_koi:icemachine:addWater', function()
    local src = source
    ...
    if #(distance - Location.TargetCoords.IceMachine[1].coords) <= 5 then
        ...
        print(('^1[Exploit Attempt]^0 %s (%s) tried to trigger event: pl_koi:icemachine:addWater.'):format(PlayerName, Identifier))
    end
end)


RegisterServerEvent('pl_koi:icemachine:getStatus')
AddEventHandler('pl_koi:icemachine:getStatus', function()
    local src = source
    local Identifier = GetPlayerIdentifier(src)
    local PlayerName = getPlayerName(src)
    local ped = GetPlayerPed(src)
    ...
    if #(distance - Location.TargetCoords.IceMachine[1].coords) <= 5 then
        ...
        print(('^1[Exploit Attempt]^0 %s (%s) tried to trigger event: pl_koi:icemachine:getStatus.'):format(PlayerName, Identifier))
    end
end)

RegisterServerEvent('pl_koi:icemachine:takeIce')
AddEventHandler('pl_koi:icemachine:takeIce', function()
    local src = source
    local Identifier = GetPlayerIdentifier(src)
    local PlayerName = getPlayerName(src)
    local ped = GetPlayerPed(src)
    ...
    if #(distance - Location.TargetCoords.IceMachine[1].coords) <= 5 then
        ...
        print(('^1[Exploit Attempt]^0 %s (%s) tried to trigger event: pl_koi:icemachine:takeIce.'):format(PlayerName, Identifier))
    end
end)
