...
function SpawnDeliveryVehicle()
    ...
    TriggerServerEvent("pl_uwucafe:generateOrders")
...
function openOrderDetails(order)
    ...
    if order.status == "Pending" then
        table.insert(menuOptions, {
            ...
            iconColor = "green",
            onSelect = function()
                TriggerServerEvent('pl_uwucafe:acceptOrder', order)
        ...
        iconColor = "red",
        onSelect = function() 
            TriggerServerEvent('pl_uwucafe:declineOrder', order)
            if Config.ContextMenu == 'ox_lib' then
                lib.hideContext()
            elseif Config.ContextMenu == 'lation_ui' then
                exports.lation_ui:hideMenu()
    ...
    elseif order.status == "Accepted" then
        table.insert(menuOptions, {
            ...
            onSelect = function()
                activeOrder = order
                TriggerServerEvent('pl_uwucafe:setOrderEnroute', order)
...
function setupZoneForDelivery(location, customer,order)
    ...
    function inside(self)
        if IsControlJustPressed(0, 38) then
            TriggerServerEvent('pl_uwucafe:deliverOrder', order)
            RemoveBlip(JobBlip)
            TextUIHide()
            self:remove() 
        end
...
RegisterNetEvent('pl_uwucafe:displayOrdersMenu')
AddEventHandler('pl_uwucafe:displayOrdersMenu', function(orders)
    displayActiveOrders(orders)
end)

RegisterCommand('uwushoworders', function()
    TriggerServerEvent('pl_uwucafe:getOrders')
end, false)
