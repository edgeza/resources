...
CreateThread(function()
    local state = GetResourceState('pma-voice')
    while state == 'starting' do
        Wait(50)
        state = GetResourceState('pma-voice')
    end
    if state ~= 'started' then return end
    local function resolveRadioName(src)
        ...
        local metadata = player.metadata or player.PlayerData and player.PlayerData.metadata
        local charinfo = player.charinfo or player.PlayerData and player.PlayerData.charinfo

        local callsign = metadata and (trim(metadata.callsign) or trim(metadata.call_sign))
        if callsign then return callsign end

        local fullname
        if charinfo then
            fullname = trim(('%s %s'):format(charinfo.firstname or '', charinfo.lastname or ''))
        end
        if fullname and fullname ~= '' then return fullname end

        local fallback = trim(player.name or (player.PlayerData and player.PlayerData.name))
        if fallback and fallback ~= '' then return fallback end

        return GetPlayerName(src)
    end

    exports['pma-voice']:overrideRadioNameGetter(function(source)
        local ok, result = pcall(resolveRadioName, source)
...
RegisterNetEvent('mm_radio:server:addToRadioChannel', function(channel, username)
    ...
    if player and player.state then
        player.state:set('radioName', username, true)
    end

    if GetResourceState('pma-voice') == 'started' then
        TriggerClientEvent('pma-voice:updateRadioName', -1, src, username)
    end
end)
