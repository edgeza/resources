...
function Radio:GetPreferredName(player)
    player = player or Framework.core.getPlayerData()
    if not player then return nil end

    local metadata = player.metadata or (player.PlayerData and player.PlayerData.metadata) or {}
    local callsign = trim(metadata and metadata.callsign or '')
    if callsign ~= '' then
        return callsign
    end

    local firstName = player.firstName or player.firstname
    local lastName = player.lastName or player.lastname

    if (not firstName or firstName == '') and player.charinfo then
        firstName = player.charinfo.firstname
    end
    if (not lastName or lastName == '') and player.charinfo then
    ...
    local fallback = trim(player.name or '')
    if fallback ~= '' then
        return fallback
    end
...
function Radio:SyncChannelName(name)
    if not name or name == '' then return end
    if self.RadioChannel and self.RadioChannel ~= 0 then
        TriggerServerEvent('mm_radio:server:addToRadioChannel', self.RadioChannel, name)
    end
end

function Radio:SetAutoName(name)
    if not name or name == '' then return end
    if not self.identifier or not self.userData[self.identifier] then return end

    local userData = self.userData[self.identifier]
    if userData.customName then
...
function Radio:Init(data)
    ...
    self.userData[self.identifier].customName = self.userData[self.identifier].customName or false
    local preferredName = self:GetPreferredName(player) or (player.firstName .. " " .. player.lastName)
    if preferredName then
        self:SetAutoName(preferredName)
    end
    local rec = {}
...
function Radio:doRadioCheck(items)
    ...
    -- Ensure items argument is a table; fallback to framework inventory otherwise
    if type(items) ~= 'table' then
        items = nil
    end

    local playerItems = items or Framework.inventory.playerItems() or {}
    ...
    for _, v in pairs(playerItems) do
        if type(v) == 'table' and lib.table.contains(Shared.RadioItem, v.name) then
            self.hasRadio = true
            local meta = Shared.Inventory == 'ox' and v.metadata or v.info
            if meta and meta.radioId then
                self.batteryData[#self.batteryData+1] = meta.radioId
            end
        end
    end
