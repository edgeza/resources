...
RegisterNetEvent('pl_uwucafe:Ingredientshop')
AddEventHandler('pl_uwucafe:Ingredientshop', function(paymentMode,itemName,label, itemAmount, price)
    local src = source
...
RegisterNetEvent('pl_uwucafe:getStock')
AddEventHandler('pl_uwucafe:getStock', function()
    local src = source
    local Identifier = GetPlayerIdentifier(src)
    local PlayerName = getPlayerName(src)
    ...
    if #(distance - Location.TargetCoords.Fridge[1].coords) <= 5 then
        if GetJob(src) == Config.Jobname then
            TriggerClientEvent('pl_uwucafe:returnStock', src, stock)
        else
            print('Tried to open the stock menu without having job. Most probably a Cheater '..Identifier..' ')
        end
    else
        print(('^1[Exploit Attempt]^0 %s (%s) tried to trigger event: pl_uwucafe:getStock.'):format(PlayerName, Identifier))
    end
end)

lib.callback.register('pl_uwucafe:getShopStatus', function(source)
    local result = MySQL.Sync.fetchScalar('SELECT state FROM pl_uwucafe LIMIT 1')
    return result == 'open'
end)

RegisterNetEvent('pl_uwucafe:toggleShopStatus', function(newState)
    local stateText = newState and 'open' or 'close'
    MySQL.Async.execute('UPDATE pl_uwucafe SET state = ?', { stateText }, function(rowsChanged)end)
end)

function ServerNotify(src, msg, type)
  TriggerClientEvent("pl_uwucafe:client:notify", src, msg, type)
end
