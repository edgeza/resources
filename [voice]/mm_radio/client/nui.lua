...
RegisterNUICallback('saveData', function(data, cb)
    ...
    local inputName = type(data.name) == 'string' and data.name:match('^%s*(.-)%s*$') or ''
    if inputName == '' then
        Radio.userData[Radio.identifier].customName = false
        local fallback = Radio:GetPreferredName(player)
        if fallback then
            Radio:SetAutoName(fallback)
        end
        SetResourceKvp('radioSettings2', json.encode(Radio.userData))
    else
        Radio.userData[Radio.identifier].customName = true
        if Radio.userData[Radio.identifier].name ~= inputName then
            Radio.userData[Radio.identifier].name = inputName
            Radio:update()
            Radio:SyncChannelName(inputName)
        end
        SetResourceKvp('radioSettings2', json.encode(Radio.userData))
    end
    cb(retreval)
