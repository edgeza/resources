...
--  ▀▄▀ █ ▀█ █▄▄  █    █▀ ▀▄▀ █▀▄   █▄▄ ▄██ █ █
if Config.Core == "ESX" then
    RegisterNetEvent('vms_flightschoolv2:sv:loadLicensesWithRestartScript', function()
        local src = source
        local xPlayer = SV.getPlayer(src)
        if xPlayer then
            TriggerEvent('esx_license:getLicenses', src, function(licenses)
                TriggerClientEvent('vms_flightschoolv2:cl:getLicenses', src, licenses)
            end)
        end
    end)
...
--  ▄▀▄ █▀▄ █▀▄   █   █ ▄▀▀ ██▀ █▄ █ ▄▀▀ ██▀
--  █▀█ █▄▀ █▄▀   █▄▄ █ ▀▄▄ █▄▄ █ ▀█ ▄██ █▄▄
RegisterNetEvent('vms_flightschoolv2:sv:addLicense', function(passed, type)
    local src = source
    ...
    if passed then
        if Config.LicensesResource == 'buty-license' then
            ...
            TriggerClientEvent('vms_flightschoolv2:notification', src, TRANSLATE('success_practical'), 'success')
        else
            if Config.Core == "ESX" then
                TriggerEvent('esx_license:addLicense', src, type, function()
                    TriggerEvent('esx_license:getLicenses', src, function(licenses)
                        TriggerClientEvent('vms_flightschoolv2:cl:getLicenses', src, licenses)
                        if Config.AddLicenseItem and (type == Config.Licenses.Practical["HELICOPTER"].name or type == Config.Licenses.Practical["PLANE"].name) then
                            local xPlayer = SV.getPlayer(src)
                            ...
                            for k, v in pairs(licenses) do
                                if v.type == Config.Licenses.Practical["HELICOPTER"].name then
                                    types = types .. "HELICOPTER "
                                elseif v.type == Config.Licenses.Practical["PLANE"].name then
                                    types = types .. "PLANE "
                                end
                            end
                            --#
            ...
            elseif Config.Core == "QB-Core" then
                ...
                if Config.AddLicenseItem and (type == Config.Licenses.Practical["HELICOPTER"].name or type == Config.Licenses.Practical["PLANE"].name) then
                    ...
                    SV.addItem(src, xPlayer, Config.LicenseItem, 1, {
                        ...
                        type = (licenses[Config.Licenses.Practical["HELICOPTER"].name] and "HELICOPTER" or "") .. (licenses[Config.Licenses.Practical["PLANE"].name] and "PLANE" or "")  -- HERE YOU CAN ADJUST METADATA
                    })
                end
                xPlayer.Functions.Save()
                TriggerClientEvent('vms_flightschoolv2:cl:getLicenses', src)
                TriggerClientEvent('vms_flightschoolv2:notification', src, TRANSLATE('success_practical'), 'success')
            end
        end
    else
        TriggerClientEvent('vms_flightschoolv2:notification', src, TRANSLATE('failed_practical'), 'error')
    end
