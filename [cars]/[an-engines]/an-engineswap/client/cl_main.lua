    ...
    if vehicle then
        SetTimeout(500, function ()
            ...
            if engineSwap and engineSwap.exhaust then
                ...
                -- Apply sound immediately after setting state
                SetTimeout(200, function()
                    if DoesEntityExist(vehicle) and GetEntityType(vehicle) == 2 then
                        pcall(function()
                            ForceVehicleEngineAudio(vehicle, engineSwap.exhaust)
                            cacheSound[vehicle] = engineSwap.exhaust -- Update cache
                        end)
                    end
                end)
            end
...
RegisterNetEvent('an-engineswap:client:applySoundNow', function(netId, engineSound)
    ...
    if not DoesEntityExist(vehicle) then 
        -- Wait a bit and try again in case of network delay
        SetTimeout(1000, function()
            local retryVehicle = NetworkGetEntityFromNetworkId(netId)
            if DoesEntityExist(retryVehicle) then
                applyEngineSoundWithRetry(retryVehicle, engineSound, 3)
            end
        end)
        return 
    end
    
    applyEngineSoundWithRetry(vehicle, engineSound, 3)
    -- Notify the player
    Utils.Notify({msg = "Engine sound successfully installed!", type = "success", duration = 5000})
end)

-- Helper function to apply engine sound with retry mechanism
function applyEngineSoundWithRetry(vehicle, engineSound, maxRetries)
    if not DoesEntityExist(vehicle) or not engineSound then return end
    
    local attempts = 0
    local function tryApplySound()
        attempts = attempts + 1
        
        -- Validate vehicle state before applying sound
        if not DoesEntityExist(vehicle) or GetEntityType(vehicle) ~= 2 then
            return
        end
        
        -- Check if vehicle is ready for sound changes
        if not IsVehicleDriveable(vehicle, true) then
            if attempts < maxRetries then
                SetTimeout(500 * attempts, tryApplySound) -- Progressive delay
            end
            return
        end
        
        -- Update the cache immediately to prevent conflicts
        cacheSound[vehicle] = engineSound
        
        -- Apply the engine sound with validation
        local success = pcall(function()
        ...
        if not success and attempts < maxRetries then
            SetTimeout(500 * attempts, tryApplySound)
        end
    end
