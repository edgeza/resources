...
function getOnDutyPoliceAmount()
    -- If another resource explicitly sets the count, prefer that value.
    ...
    if framework == 'QB' then
        ...
        elseif QBCore and QBCore.Functions and QBCore.Functions.GetPlayers then
    ...
    elseif framework == 'ESX' then
        local police = ESX.GetExtendedPlayers and ESX.GetExtendedPlayers('job', 'police') or {}
        local bcso = ESX.GetExtendedPlayers and ESX.GetExtendedPlayers('job', 'bcso') or {}
        return (#police) + (#bcso)
    else
        -- CUSTOM
        return 0
