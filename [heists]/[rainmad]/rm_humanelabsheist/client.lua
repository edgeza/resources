            ...
            if not HumaneLabsHeist['finish'] then
                ...
                if IsControlJustPressed(0, 38) and not progress then
                    ...
                    if not closestPlayer then
                        ...
                        QBCore.Functions.Notify(Strings['closest_player'])
                    end
                end
            end
        end
...
AddEventHandler('humanelabsheist:client:policeAlert', function(targetCoords)
    ...
    local alpha = 250
    local labsBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, 50.0)

    SetBlipHighDetail(labsBlip, true)
    SetBlipColour(labsBlip, 1)
    SetBlipAlpha(labsBlip, alpha)
    SetBlipAsShortRange(labsBlip, true)
...
function SpawnPeds()
    ...
    policeAlert(vector3(3611.10, 3744.44, 27.6900))
end
...
function StartMinigame()
    ...
    while RequestTaskMoveNetworkStateTransition(ped, 'Cutting') do
