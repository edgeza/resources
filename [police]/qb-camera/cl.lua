...
RegisterNetEvent('tlm:client:PedChecks', function()
    ...
    if not IsPedInAnyVehicle(PlyPed) then
        ...
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() and PlayerData.job.type == 'leo' then
