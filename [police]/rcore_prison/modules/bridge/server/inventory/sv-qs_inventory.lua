    ...
    if Config.Inventories == Inventories.QS then
        ...
        Inventory.getInventoryItems = function(playerId)
            ...
            if doesExportExistInResource(Inventories.QS, "GetPlayerInventory") then
                ...
                inventory = exports[Inventories.QS]:GetPlayerInventory(playerId)
                fetchState = true
            elseif doesExportExistInResource(Inventories.QS, "GetInventory") then
                ...
                inventory = exports[Inventories.QS]:GetInventory(playerId)
                fetchState = true
            else
                dbg.debug("Inventory-QS: Loading player inventory as fallback via Framework.")
                if Config.Framework == Framework.QBCore or Config.Framework == Framework.QBOX and player and next(player) then
                    inventory = player.PlayerData.items
                    fetchState = true
                elseif Config.Framework == Framework.ESX and player and next(player) then
