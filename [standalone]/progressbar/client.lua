...
local function createAndAttachProp(prop, ped)
    ...
    local timeout = 0
    while not DoesEntityExist(propEntity) and timeout < 100 do
        Wait(0)
        timeout = timeout + 1
    end
    
    -- Wait for entity to be networked before getting network ID
    if DoesEntityExist(propEntity) then
        timeout = 0
        while not NetworkGetEntityIsNetworked(propEntity) and timeout < 100 do
            Wait(0)
            NetworkRegisterEntityAsNetworked(propEntity)
            timeout = timeout + 1
        end
        
        if NetworkGetEntityIsNetworked(propEntity) then
    ...
    -- Fallback: delete entity and return nil if networking failed
    if DoesEntityExist(propEntity) then
...
local function ActionCleanup()
    ...
    if prop_net then
        -- Check if network ID is valid before converting
        if NetworkDoesNetworkIdExist(prop_net) then
    ...
    if propTwo_net then
        -- Check if network ID is valid before converting
        if NetworkDoesNetworkIdExist(propTwo_net) then
            local propTwoEntity = NetToObj(propTwo_net)
            if DoesEntityExist(propTwoEntity) then
                DetachEntity(propTwoEntity, true, true)
                DeleteObject(propTwoEntity)
            end
        end
    end
    prop_net = nil
