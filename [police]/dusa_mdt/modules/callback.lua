-- @@@ credit goes to https://github.com/pitermcflebor/pmc-callbacks
...
if IS_SERVER then
	...
	_G.RegisterServerCallback = function(args)
		...
		local eventData = RegisterNetEvent('pmc__server_callback:'..eventName, function(packed, src, cb)
			...
			if not source then
				...
				-- return the data
				lib.triggerClientEvent(('pmc__client_callback_response:%s:%s'):format(eventName, source), source, msgpack_pack_args( eventCallback(source, table_unpack(msgpack_unpack(packed)) ) ))
			end
		end)
	...
	_G.TriggerClientCallback = function(args)
		...
		if tonumber(args.source) >= 0 then
			...
			lib.triggerClientEvent(('pmc__client_callback:%s'):format(args.eventName), args.source, msgpack_pack(args.args or {}), ticket)
