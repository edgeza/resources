...
function RemoveItem(playerId, item, slot)
	...
	if player then 
		-- Try ox_inventory first, then fallback to qs-inventory
		if ox_inventory then
			ox_inventory:RemoveItem(playerId, item, 1, nil, slot)
		elseif qs_inventory then
			qs_inventory:RemoveItem(playerId, item, 1, nil, nil, nil, nil, true)
		end
	end
end
...
function DoesPlayerHaveItem(player, items, removeItem)
	...
	for i = 1, #items do
		...
		if ox_inventory then
			-- Method 1: Try Search function (newer versions)
			local searchResult = ox_inventory:Search(playerId, 'slots', itemName, item.metadata)
			if type(searchResult) == 'table' and #searchResult > 0 then
				data = searchResult[1]
			end
			
			-- Method 2: If Search doesn't work or returns unexpected format, use GetInventoryItems
			if not data then
				local inventoryItems = ox_inventory:GetInventoryItems(playerId)
				if inventoryItems then
					for slot, itemData in pairs(inventoryItems) do
						if itemData.name == itemName and (not item.metadata or itemData.metadata == item.metadata) then
							data = itemData
							break
						end
					end
		...
		-- Fallback to qs-inventory if ox_inventory doesn't work
		if not data and qs_inventory then
			-- Use qs-inventory's GetItemTotalAmount function to check if player has the item
			local itemAmount = qs_inventory:GetItemTotalAmount(playerId, itemName)
			if itemAmount and itemAmount > 0 then
				data = {
					name = itemName,
					count = itemAmount,
					slot = nil -- qs-inventory doesn't need slot for removal
				}
			end
		end
		...
		if data and data.count > 0 then
			if removeItem or item.remove then
				...
				elseif qs_inventory then
					qs_inventory:RemoveItem(playerId, itemName, 1, nil, nil, nil, nil, true)
				end
			end
...
local function isAuthorised(playerId, door, lockpick)
	...
	if player then
		if lockpick then
			-- If lockpicking, only check if player has lockpick item
			-- Don't check other authorization methods
