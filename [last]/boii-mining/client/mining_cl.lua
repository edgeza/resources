...
local function playerHasItem(itemName)
    ...
    local pd = Core and Core.Functions and Core.Functions.GetPlayerData and Core.Functions.GetPlayerData() or nil
    local inv = pd and pd.items or {}
    if type(inv) ~= 'table' then return false end
    for _, it in pairs(inv) do
        if it and it.name == itemName and ((it.amount or it.count or 0) > 0) then
            return true
        end
    end
    return false
...
local function setThirdEyeLocked(locked)
	-- Try ox_target first
	pcall(function()
		if GetResourceState('ox_target') == 'started' then exports['ox_target']:disableTargeting(locked) end
	end)
	-- Then qb-target variants
	pcall(function()
		if GetResourceState('qb-target') == 'started' and exports['qb-target'] then
			if exports['qb-target'].DisableTargeting then exports['qb-target']:DisableTargeting(locked) end
			if exports['qb-target'].AllowTargeting then exports['qb-target']:AllowTargeting(not locked) end
			if exports['qb-target'].ToggleTarget then exports['qb-target']:ToggleTarget(not locked) end
		end
	end)
end
    ...
    for k, v in pairs(Config.Peds) do
        if v.useped then
            ...
            if MiningPeds and MiningPeds ~= 0 and DoesEntityExist(MiningPeds) then
                ...
                -- Ensure qb-target is ready before adding entity zone
                local waitMs = 0
        ...
        exports[TargetName]:addLocalEntity(obj, {
            {
                ...
                onSelect = function()
                    ...
                    if entity and DoesEntityExist(entity) then
                        removedPos = GetEntityCoords(entity)
                        safeDelete(entity)
...
RegisterNetEvent('boii-mining:cl:PlaceDynamite', function(ctx)
    ...
    if Config and Config.XP and Config.XP.Use then
        local pdata = Core and Core.Functions and Core.Functions.GetPlayerData and Core.Functions.GetPlayerData() or nil
        local miningXp = (pdata and pdata.metadata and pdata.metadata[Config.XP.MetaDataName]) or 0
...
CreateThread(function()
    while true do
        ...
        if IsPedSwimming(ped) then
            nearWater = true
        elseif TestProbeAgainstWater(coords.x, coords.y, coords.z + 1.0, coords.x, coords.y, coords.z - 5.0) then
            nearWater = true
        end
...
RegisterNetEvent('boii-mining:cl:OpenCraftMenu', function(shop)
    -- qb-menu expects an array of entries; pass through the built list
    ...
    exports[MenuName]:openMenu(shop.items or {})
end)
--<!>-- CLIENT CRAFT MENU OPEN --<!>--
...
if Config.XP and Config.XP.Use and Config.XP.Command then
    ...
    RegisterCommand('miningxp', function()
        ...
        local total = (pd and pd.metadata and pd.metadata[metaName]) or 0
