...
local function initializeVehiclesHash()
    if not VEHICLES_HASH or next(VEHICLES_HASH) == nil then
        VEHICLES_HASH = exports.qbx_core:GetVehiclesByHash()
    end
end
...
lib.callback.register('qbx_admin:client:GetVehicleInfo', function()
    ...
    if not VEHICLES_HASH[vehicleModel] then
        return nil, nil
    end
    
    return VEHICLES_HASH[vehicleModel].model, lib.getVehicleProperties(cache.vehicle)
end)
