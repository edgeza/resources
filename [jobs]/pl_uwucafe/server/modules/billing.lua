            ...
            if billed ~= nil then
                if getPlayerName(source) ~= getPlayerName(source) then
                    if amount and amount > 0 then
                        ...
                        TriggerClientEvent("pl_uwucafe:notification", source, Locale("invoice_sent"), 'success')
                        TriggerClientEvent("pl_uwucafe:notification", billed.PlayerData.source, Locale("invoice_recived"))
                    else
                        TriggerClientEvent("pl_uwucafe:notification", source, Locale("must_zero"), 'error')
                    end
                else
                    TriggerClientEvent("pl_uwucafe:notification", source, Locale("cannot_bill_yourself"), 'error')
                end
            else
                TriggerClientEvent("pl_uwucafe:notification", source,Locale("player_not_online"), 'error')
            end
        else
            TriggerClientEvent("pl_uwucafe:notification", source, Locale("no_access"), 'error')
        end
end

RegisterServerEvent("pl_uwucafe:billplayer")
AddEventHandler("pl_uwucafe:billplayer", function(targetId, billamount,can)
    local src = source
