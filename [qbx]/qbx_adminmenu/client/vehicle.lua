...
local function initializeCoreVehicles()
    if not coreVehicles or next(coreVehicles) == nil then
        coreVehicles = exports.qbx_core:GetVehiclesByName()
    end
end

function GenerateVehiclesSpawnMenu()
    initializeCoreVehicles()
    ...
    for i = 1, #categories do
        ...
        lib.registerMenu({
            ...
            -- Give keys through all available vehicle key systems
            VehicleKeysIntegration.ClientGiveKeys(qbx.getVehiclePlate(veh))
...
lib.registerMenu({
    ...
    options = {
        ...
        {label = 'Tune Vehicle'},
        {label = 'Change Plate'},
        {label = 'Give Vehicle Keys', close = false},
        {label = 'Remove Vehicle Keys', close = false}
    }
}, function(selected)
    if selected == 1 then
    ...
    elseif selected == 6 then
        ...
        -- Give keys through all available vehicle key systems
        VehicleKeysIntegration.ClientGiveKeys(dialog[1])
    elseif selected == 7 then
        ...
        if not players or #players == 0 then
            exports.qbx_core:Notify('No players found', 'error')
            lib.showMenu('qbx_adminmenu_vehicles_menu', MenuIndexes.qbx_adminmenu_vehicles_menu)
            return
        end
        ...
        for _, player in pairs(players) do
            table.insert(playerOptions, {label = player.name .. ' (ID: ' .. player.id .. ')', args = {player.id}})
        end

        lib.registerMenu({
            id = 'qbx_adminmenu_give_keys_menu',
            ...
            onSelected = function(selected)
                MenuIndexes.qbx_adminmenu_give_keys_menu = selected
            end,
            ...
            -- Give keys through both systems
            TriggerServerEvent('qbx_admin:server:giveVehicleKeys', targetId, plate)
            
            exports.qbx_core:Notify('Vehicle keys given to player', 'success')
            lib.showMenu('qbx_adminmenu_vehicles_menu', MenuIndexes.qbx_adminmenu_vehicles_menu)
        end)

        lib.showMenu('qbx_adminmenu_give_keys_menu', MenuIndexes.qbx_adminmenu_give_keys_menu)
    elseif selected == 8 then
        ...
        lib.registerMenu({
            id = 'qbx_adminmenu_remove_keys_menu',
            title = 'Remove Vehicle Keys',
            position = 'top-right',
            onClose = function(keyPressed)
                CloseMenu(false, keyPressed, 'qbx_adminmenu_vehicles_menu')
            end,
            onSelected = function(selected)
                MenuIndexes.qbx_adminmenu_remove_keys_menu = selected
            end,
            options = playerOptions
        }, function(_, _, args)
            local targetId = args[1]
            ...
            -- Remove keys through both systems
            TriggerServerEvent('qbx_admin:server:removeVehicleKeys', targetId, plate)
            
            exports.qbx_core:Notify('Vehicle keys removed from player', 'success')
            lib.showMenu('qbx_adminmenu_vehicles_menu', MenuIndexes.qbx_adminmenu_vehicles_menu)
        end)

        lib.showMenu('qbx_adminmenu_remove_keys_menu', MenuIndexes.qbx_adminmenu_remove_keys_menu)
    end
end)
