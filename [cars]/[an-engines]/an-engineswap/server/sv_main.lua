...
RegisterNetEvent('an-engine:server:install', function (data)
    ...
    SetTimeout(500, function()
        -- Double-check the vehicle still exists
        local checkVehicle = NetworkGetEntityFromNetworkId(netId)
        if DoesEntityExist(checkVehicle) and GetEntityType(checkVehicle) == 2 then
            TriggerClientEvent('an-engineswap:client:applySoundNow', source, netId, engine)
        end
    end)
end)
