...
lib.callback.register('qbx_admin:server:spawnVehicle', function(source, model)
    ...
    VehicleKeysIntegration.GiveKeys(source, plate, vehicle)
...
-- Vehicle Keys Management Events
RegisterNetEvent('qbx_admin:server:giveVehicleKeys', function(targetId, plate)
    if not IsPlayerAceAllowed(source, config.eventPerms.useMenu) then 
        exports.qbx_core:Notify(source, locale('error.no_perms'), 'error') 
        return 
    end
    ...
    if not target then
        exports.qbx_core:Notify(source, 'Target player not found', 'error')
        return
    end

    -- Give keys through all available vehicle key systems
    VehicleKeysIntegration.GiveKeys(targetId, plate)

    exports.qbx_core:Notify(source, 'Vehicle keys given to ' .. target.PlayerData.charinfo.firstname .. ' ' .. target.PlayerData.charinfo.lastname, 'success')
    exports.qbx_core:Notify(targetId, 'You received keys for vehicle: ' .. plate, 'success')
end)

RegisterNetEvent('qbx_admin:server:removeVehicleKeys', function(targetId, plate)
    if not IsPlayerAceAllowed(source, config.eventPerms.useMenu) then 
        exports.qbx_core:Notify(source, locale('error.no_perms'), 'error') 
        return 
    end
    ...
    -- Remove keys through all available vehicle key systems
    VehicleKeysIntegration.RemoveKeys(targetId, plate)

    exports.qbx_core:Notify(source, 'Vehicle keys removed from ' .. target.PlayerData.charinfo.firstname .. ' ' .. target.PlayerData.charinfo.lastname, 'success')
    exports.qbx_core:Notify(targetId, 'Your keys for vehicle: ' .. plate .. ' have been removed', 'error')
end)