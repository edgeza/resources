			...
			MySQL.Async.execute('INSERT INTO dusa_pets (owner, modelname, type) VALUES (@owner, @modelname, @type)',
				{
					...
					TriggerClientEvent('dusa_pets:cl:notify', _source, Config.Notifications[Config.Locale].bought_pet,
						'success')
...
function CheckMoney(xPlayer, price, moneyType, _source)
	if moneyType == "cash" then
		if dusa.getMoney(_source, 'money') > price then
			...
			TriggerClientEvent('dusa_pets:cl:notify', _source, Config.Notifications[Config.Locale].not_enough_cash,
				'error')
			return false
		end
	elseif moneyType == 'card' then
		if dusa.getMoney(_source, 'bank') > price then
			...
			TriggerClientEvent('dusa_pets:cl:notify', _source, Config.Notifications[Config.Locale].not_enough_bank,
				'error')
			return false
		end
	end
...
AddEventHandler('dusa_pets:sv:updatePet', function(health, illness, pet)
	...
	MySQL.Async.execute(
	'UPDATE dusa_pets SET health = @health, illnesses = @illness WHERE owner = @owner AND modelname = @modelname', {
		['@health']    = health,
		['@illness']   = illness,
		['@owner']     = identifier,
		['@modelname'] = pet
...
RegisterServerEvent('dusa-pets:returnBall')
AddEventHandler('dusa-pets:returnBall', function(type)
	local source = source
	local player = dusa.getPlayer(source)
	if player then
		if type == 'add' then
			dusa.addItem(source, 'tennisball', 1)
		else
			dusa.removeItem(source, 'tennisball', 1)
		end
	end
end)
...
lib.callback.register('dusa_pets:cb:canBuyItem', function(source, data)
    local src = source
    local total = 0
    local basket = data.data
    local ped = GetPlayerPed(src)
    local pedCoords = GetEntityCoords(ped)
    local dist = #(pedCoords - Config.Peds['petstore'].coords)

    if dist > 10 then
        if Config.DropExploiter then
            DropPlayer(src, 'Attempted to exploit')
        end
        return 0
    end
    ...
    for i = 1, #basket do
        if basket[i] then
            local playerMoney = dusa.getMoney(src, 'money')

            if data.account == "cash" then
                if playerMoney >= basket[i].price then
                    dusa.addItem(src, basket[i].item, basket[i].count)
                    dusa.removeMoney(src, 'money', basket[i].totalPrice, 'Pet Store Purchase (Cash)')
                    total = total + basket[i].totalPrice
                else
                    TriggerClientEvent('dusa_pets:cl:notify', src, Config.Notifications[Config.Locale].not_enough_cash, 'error')
                    return 0
                end
            elseif data.account == 'card' then
                local playerBank = dusa.getMoney(src, 'bank')

                if playerBank >= basket[i].price then
                    dusa.addItem(src, basket[i].item, basket[i].count)
                    dusa.removeMoney(src, 'bank', basket[i].totalPrice, 'Pet Store Purchase (Bank)')
                    total = total + basket[i].totalPrice
                else
                    TriggerClientEvent('dusa_pets:cl:notify', src, Config.Notifications[Config.Locale].not_enough_bank, 'error')
                    return 0
                end
            end
        end
...
RegisterNetEvent("dusa_objects:server:CreateNewObject", function(model, coords, objecttype, options, objectname)
	...
	if model and coords then
		TriggerClientEvent("dusa_objects:client:AddObject", -1,
			{ id = math.random(0, 10000), model = model, coords = coords, type = objecttype, name = objectname, options =
			options })
	else
...
function GetStatus(identifier, model)
	local result = MySQL.query.await("SELECT health, illnesses FROM dusa_pets WHERE owner = '" ..
	identifier .. "' AND modelname = '" .. model .. "'")
	return result[1]
end
...
function GetPetName(identifier, model)
	local result = MySQL.query.await("SELECT name FROM dusa_pets WHERE owner = '" ..
	identifier .. "' AND modelname = '" .. model .. "'")
	return result[1].name
end

function GetPetClothes(identifier, model)
	local result = MySQL.query.await("SELECT clothes FROM dusa_pets WHERE owner = '" ..
	identifier .. "' AND modelname = '" .. model .. "'")
	return result[1]
end
...
AddEventHandler('dusa-pets:sv:saveClothes', function(data, model)
	...
	local player = dusa.getPlayer(source)
	local identifier = dusa.getIdentifier(source)
	local result = ExecuteSql("SELECT clothes FROM dusa_pets WHERE owner = '" ..
	identifier .. "' AND modelname ='" .. model .. "'")
	if result[1] ~= nil then
		MySQL.update('UPDATE dusa_pets SET `clothes` = ? WHERE owner = ? AND modelname = ?',
			{ json.encode(data), dusa.getIdentifier(source), model })
	end
end)
