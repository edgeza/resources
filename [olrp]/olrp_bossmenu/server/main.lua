...
-- Get job data
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetJobData', function(source, cb, jobName)
    local src = source
...
function StartRefreshTimer(src, jobName)
    ...
    RefreshTimers[src] = SetTimeout(interval, function()
        ...
        if Player and Player.PlayerData.job.name == jobName then
            ...
            TriggerClientEvent('olrp-bossmenu:client:RefreshData', src)
...
-- Get user settings
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetSettings', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('olrp-bossmenu:server:UpdateEmployee', function(citizenid, jobName, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
local gradeNum = grade
if type(grade) == 'string' then
    gradeNum = tonumber(grade)
    if not gradeNum then
        TriggerClientEvent('QBCore:Notify', src, "Invalid job grade format", "error")
        return
    end
end

-- Check if grade exists for this job
if not QBCore.Shared.Jobs[jobName].grades[gradeNum] then
    ...
    return
        ...
        if targetPlayer then
            ...
            if targetCurrentJob == jobName then
                ...
                targetPlayer.Functions.SetJob(jobName, gradeNum)
                TriggerClientEvent('QBCore:Notify', targetPlayer.PlayerData.source, "Your rank has been updated to " .. QBCore.Shared.Jobs[jobName].grades[gradeNum].name, "success")
                ...
                TriggerEvent('qb-log:server:CreateLog', 'jobmanagement', 'Employee Rank Update', 'green', string.format('%s (%s) updated %s (%s) to rank %s in job %s', 
                    GetPlayerName(src), Player.PlayerData.citizenid, GetPlayerName(targetPlayer.PlayerData.source), citizenid, gradeNum, jobName))
            else
                targetPlayer.Functions.SetJob(jobName, gradeNum)
                ...
                TriggerEvent('qb-log:server:CreateLog', 'jobmanagement', 'Job Change', 'green', string.format('%s (%s) changed %s (%s) job from %s to %s at rank %s', 
                    GetPlayerName(src), Player.PlayerData.citizenid, GetPlayerName(targetPlayer.PlayerData.source), citizenid, targetCurrentJob, jobName, gradeNum))
            end
        else
            ...
            MySQL.Async.fetchSingle('SELECT job FROM players WHERE citizenid = @citizenid', {
                ...
                local jobInfo = {
                    ...
                    payment = QBCore.Shared.Jobs[jobName].grades[gradeNum].payment,
                    onduty = true,
                    grade = {
                        level = gradeNum,
                        name = QBCore.Shared.Jobs[jobName].grades[gradeNum].name
                    },
                    isboss = QBCore.Shared.Jobs[jobName].grades[gradeNum].isboss or false
                }
...
-- Hire new employee
RegisterNetEvent('olrp-bossmenu:server:HireEmployee', function(targetId, jobName, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    -- Set target job
    Target.Functions.SetJob(jobName, gradeNum)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'jobmanagement', 'Employee Hire', 'green', string.format('%s (%s) hired %s (%s) for job %s at rank %s', 
        GetPlayerName(src), Player.PlayerData.citizenid, GetPlayerName(Target.PlayerData.source), Target.PlayerData.citizenid, jobName, gradeNum))
end)

-- Remove employee from job
RegisterNetEvent('olrp-bossmenu:server:RemoveEmployee', function(citizenid, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    MySQL.Async.fetchSingle('SELECT job FROM players WHERE citizenid = @citizenid', {
        ...
        if targetPlayer then
            ...
            TriggerClientEvent('olrp-bossmenu:client:EmployeeFired', src, true, "Employee fired successfully")
            ...
            MySQL.Async.execute('UPDATE players SET job = @job WHERE citizenid = @citizenid', {
                ...
                if rowsChanged > 0 then
                    ...
                    TriggerClientEvent('olrp-bossmenu:client:EmployeeFired', src, false, "Failed to fire employee")
                end
            end)
        end
    end)
...
-- Save user settings
RegisterNetEvent('olrp-bossmenu:server:SaveSettings', function(settings)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('QBCore:Server:OnJobUpdate', function(source, newJob)
    ...
    if Player then
        ...
        TriggerClientEvent('olrp-bossmenu:client:JobChanged', -1, newJob.name)
    end
end)
...
QBCore.Commands.Add('managejob', 'Open job management interface', {}, false, function(source)
    ...
    TriggerClientEvent('olrp-bossmenu:client:TriggerOpenManager', source, {jobData = jobName})
end)

RegisterNetEvent('olrp-bossmenu:server:RefreshPlayTime', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
function LoadSocietyTransactions()
    ...
    if result and #result > 0 then
        ...
        print("^2[OLRP-bossmenu]^7 No society transactions found in database")
    end
end
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetSocietyData', function(source, cb, jobName)
    local src = source
...
RegisterNetEvent('olrp-bossmenu:server:DepositMoney', function(amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('olrp-bossmenu:server:WithdrawMoney', function(amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('olrp-bossmenu:server:TransferMoney', function(citizenid, amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('olrp-bossmenu:server:SubmitApplication', function(jobName, answers)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetJobApplications', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('olrp-bossmenu:server:UpdateApplicationStatus', function(applicationId, status, notes)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:CheckPendingApplication', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:CheckApplicationStatus', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetEmployeePermissions', function(source, cb, citizenid, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:UpdateEmployeePermissions', function(source, cb, citizenid, jobName, permissions)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    MySQL.Async.execute('INSERT INTO job_employee_permissions (citizenid, job, permissions, granted_by) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE permissions = ?, granted_by = ?', 
        ...
        if rowsChanged > 0 then
            ...
            local targetPlayer = QBCore.Functions.GetPlayerByCitizenId(citizenid)
            if targetPlayer then
                TriggerClientEvent('olrp-bossmenu:client:RefreshPermissions', targetPlayer.PlayerData.source, permissions)
            ...
            for _, p in pairs(players) do
                if p.PlayerData.job.name == jobName then
                    TriggerClientEvent('olrp-bossmenu:client:SyncPermissions', p.PlayerData.source, citizenid, permissions)
                end
            end
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:HireNewEmployee', function(source, cb, targetId, jobName, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayer(tonumber(targetId))
    ...
    if type(grade) == 'string' then
        gradeNum = tonumber(grade)
        if not gradeNum then
            cb({ success = false, message = "Invalid job grade format" })
            return
        end
    end
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:HasJobAccess', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetPlaytimeData', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('olrp-bossmenu:server:GetJobGrades', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('olrp-bossmenu:server:RequestRefreshJobData', function()
    TriggerEvent('QBCore:server:UpdateObject')
end)
