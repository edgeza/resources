...
function lockInv(toggle)
	if toggle then LocalPlayer.state:set("inv_busy", true, true) TriggerEvent('codem-inventory:client:busy:status', true) TriggerEvent('canUseInventoryAndHotbar:toggle', false)
	else LocalPlayer.state:set("inv_busy", false, true) TriggerEvent('codem-inventory:client:busy:status', false) TriggerEvent('canUseInventoryAndHotbar:toggle', true) end
end
...
function progressBar(data)
	...
	if Config.ProgressBar == "ox" then
		...
		Core.Functions.Progressbar("mechbar", data.label, data.time, data.dead, data.cancel,
			...
			{ animDict = data.dict, anim = data.anim, flags = data.flag, task = data.task }, {}, {}, function(cancelled)
				result = true
		end, function()
			result = false
		end, data.icon)
