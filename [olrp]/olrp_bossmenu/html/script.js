...
function setupEventListeners() {
    ...
    document.getElementById('fire-employee-btn').addEventListener('click', function() {
        ...
        if (!citizenid || !employeeName) {
            showNotification('Employee data not available', 'error');
            return;
        }
        
        // Show the custom fire menu
        showCustomFireMenu(employeeName, citizenid);
    });
...
function setupAdditionalEventListeners() {
    document.getElementById('fire-confirm-btn').addEventListener('click', function() {
        ...
        .then(result => {
            if (result.success) {
                showNotification('Employee fired successfully', 'success');
                hideCustomFireMenu();
                // Refresh the employee data
                refreshJobData();
            } else {
                showNotification(result.message || 'Failed to fire employee', 'error');
            }
        })
        .catch(error => {
...
function updatePermissionsVisibility(permissions, isBoss) {
    ...
    // Check if user is the highest rank (CEO/Owner)
    let highestGrade = 0;
    let currentPlayerGrade = 0;
    
    if (currentJobData && currentJobData.grades) {
        // Find the highest grade number
        Object.keys(currentJobData.grades).forEach(grade => {
        ...
        // Get current player's grade (if available in job data)
        if (currentJobData.currentPlayerGrade !== undefined) {
            currentPlayerGrade = currentJobData.currentPlayerGrade;
        }
    }
    
    // If user is the highest rank, give them full access
    const hasFullAccess = isBoss && currentPlayerGrade >= highestGrade;
    ...
    if (!isBoss) {
        ...
        // Hide tabs based on specific permissions
        if (!permissions.dashboard && !hasFullAccess) {
            const dashboardTab = document.querySelector('.nav-item[data-page="dashboard"]');
            if (dashboardTab) dashboardTab.style.display = 'none';
        } else {
            const dashboardTab = document.querySelector('.nav-item[data-page="dashboard"]');
        ...
        if (!permissions.employees && !hasFullAccess) {
            const employeesTab = document.querySelector('.nav-item[data-page="employees"]');
            if (employeesTab) employeesTab.style.display = 'none';
        } else {
            const employeesTab = document.querySelector('.nav-item[data-page="employees"]');
        ...
        // Check hiring/firing permission for employee management access
        if (!permissions.hiringfiring && !hasFullAccess) {
            // Hide employee management features if no hiring/firing permission
            const hireBtn = document.getElementById('hire-employee-btn');
            if (hireBtn) hireBtn.style.display = 'none';
            
            // Hide fire buttons in employee tables
            document.querySelectorAll('.fire-employee-btn').forEach(btn => {
                btn.style.display = 'none';
            });
        } else {
            // Show employee management features if hiring/firing permission
            const hireBtn = document.getElementById('hire-employee-btn');
            if (hireBtn) hireBtn.style.display = '';
            
            // Show fire buttons in employee tables
            document.querySelectorAll('.fire-employee-btn').forEach(btn => {
                btn.style.display = '';
            });
        }
        
        if (!permissions.society && !hasFullAccess) {
            const societyTab = document.querySelector('.nav-item[data-page="society"]');
            if (societyTab) societyTab.style.display = 'none';
        } else {
            const societyTab = document.querySelector('.nav-item[data-page="society"]');
        ...
        if (!permissions.applications && !hasFullAccess) {
            const applicationsTab = document.querySelector('.nav-item[data-page="applications"]');
            if (applicationsTab) applicationsTab.style.display = 'none';
        } else {
            const applicationsTab = document.querySelector('.nav-item[data-page="applications"]');
        ...
        if (!permissions.grades && !hasFullAccess) {
            const gradesTab = document.querySelector('.nav-item[data-page="grades"]');
            if (gradesTab) gradesTab.style.display = 'none';
        } else {
            const gradesTab = document.querySelector('.nav-item[data-page="grades"]');
        ...
        // Bosses always have full access
        document.querySelectorAll('.nav-item').forEach(tab => {
            tab.style.display = '';
        });
        
        // Show all employee management features
        const hireBtn = document.getElementById('hire-employee-btn');
        if (hireBtn) hireBtn.style.display = '';
