...
local itemFunc = {
    ...
    {   script = Exports.QBXExport,
        cacheItem = function()
            -- qbx_core uses qs-inventory, try to get Items directly from inventory first
            if checkExists("qs-inventory") then
                waitStarted("qs-inventory")
                local success, result = pcall(function()
            ...
            -- Try to get Items from qbx_core bridge (exports through qb-core name)
            local coreObject = exports[Exports.QBExport]:GetCoreObject()
            if coreObject and coreObject.Shared then
                cache.Items = coreObject.Shared.Items
            end
            
            -- Wait for Items to be populated (qbx_core loads them asynchronously)
            local attempts = 0
            while (not cache.Items or not next(cache.Items)) and attempts < 50 do
                Wait(100)
                if coreObject and coreObject.Shared then
                    cache.Items = coreObject.Shared.Items
                end
            ...
            -- Set itemResource if Items were successfully loaded
            if cache.Items and next(cache.Items) then
                itemResource = Exports.QBXExport
            -- If this is nil, they need to update to qbx_core 1.23.0+
            elseif not cache.Items or not next(cache.Items) then
...
if cache.Items == nil or not next(cache.Items) then
    local resourceName = itemResource
    if itemResource == "N/A" and checkExists(Exports.QBXExport) then
        resourceName = Exports.QBXExport
    elseif itemResource == "N/A" and checkExists(Exports.QBExport) then
        resourceName = Exports.QBExport
    end
    print("^1--------------------------------------------^7")
    print("^1ERROR^7: ^1Can NOT find "..resourceName.." ^7Items ^1list^7, ^1possible error in that file or is it empty^7?")
    print("^1--------------------------------------------^7")
