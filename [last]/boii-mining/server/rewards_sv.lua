...
RegisterServerEvent('boii-mining:sv:DigPaydirt', function()
    ...
    if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[sackName] then
    ...
    if pData.Functions.AddItem(Config.Paydirt.Dirt.Return.name, 1) then
        if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[Config.Paydirt.Dirt.Return.name] then
    ...
    if Config.Paydirt.Dirt.BreakTool then
        if (Config.Paydirt.Dirt.Chance >= math.random(1, 100)) then
            ...
            if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[Config.Paydirt.Dirt.Required[1].name] then
...
RegisterServerEvent('boii-mining:sv:PanPaydirt', function()
    ...
    if (6 >= math.random(1,100)) then
        ...
        if Core and Core.Shared and Core.Shared.Items then
            if Core.Shared.Items[highreward] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[highreward], 'add', highAmt) end
            if Core.Shared.Items[regularreward2] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward2], 'add', reg2Amt) end
            if Core.Shared.Items[regularreward] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward], 'add', regAmt) end
        end
    ...
    elseif (85 >= math.random(1,100)) then
        local regAmt = 1
        pData.Functions.AddItem(regularreward, regAmt)
        if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[regularreward] then
...
RegisterServerEvent('boii-mining:sv:QuarryDrilling', function()
    ...
    if (45 >= math.random(1,100)) then
        ...
        pData.Functions.AddItem(regularreward, regAmt)
        if Core and Core.Shared and Core.Shared.Items then
            if Core.Shared.Items[highreward] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[highreward], 'add', highAmount) end
            if Core.Shared.Items[regularreward2] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward2], 'add', reg2Amt) end
            if Core.Shared.Items[regularreward] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward], 'add', regAmt) end
        end
    ...
    elseif (93 >= math.random(1,100)) then
        ...
        if Config.Quarry.Drilling.BreakTool then
            if (Config.Quarry.Drilling.Chance >= math.random(1, 100)) then
                ...
                if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[Config.Quarry.Drilling.Required.name] then
...
RegisterServerEvent('boii-mining:sv:CaveDrilling', function()
    ...
    if (60 >= math.random(1,100)) then
        ...
        if Core and Core.Shared and Core.Shared.Items then
            if Core.Shared.Items[highreward] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[highreward], 'add', highAmount) end
            if Core.Shared.Items[regularreward3] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward3], 'add', reg3Amt) end
            if Core.Shared.Items[regularreward2] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward2], 'add', reg2Amt) end
            if Core.Shared.Items[regularreward] then TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[regularreward], 'add', regAmt) end
        end
    elseif (90 >= math.random(1,100)) then
        ...
        if Config.Mine.Drilling.BreakTool then
            if (Config.Mine.Drilling.Chance >= math.random(1, 100)) then
                ...
                if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[Config.Mine.Drilling.Required.name] then
