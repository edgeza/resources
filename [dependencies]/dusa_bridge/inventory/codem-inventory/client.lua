...
Framework.OpenStash = function(name)
    ...
    Framework.TriggerCallback(Bridge.Resource .. ':bridge:GetStash', function(stash)
        ...
        TriggerServerEvent('codem-inventory:server:OpenInventory', 'stash', name, { maxweight = stash.weight, slots = stash.slots })
        TriggerEvent('codem-inventory:client:SetCurrentStash', name)
    end, name)
end
...
Framework.OpenShop = function(name)
    Framework.TriggerCallback(Bridge.Resource .. ':bridge:OpenShop', function(shopdata)
        if table.type(shopdata) ~= 'empty' then
            ...
            TriggerServerEvent("codem-inventory:server:OpenInventory", "shop", shopdata.name, Shop)
        end   
    end, name)
end

Framework.CloseInventory = function()
    exports['codem-inventory']:CloseInventory()
end
...
Framework.GetItem = function(item, metadata, strict)
    local items = {}
    ---@cast items Item[]
    local PlayerInventory = exports['codem-inventory']:GetClientPlayerInventory()
...
Framework.OpenNearbyInventory = function(playerId)
    TriggerServerEvent("codem-inventory:server:OpenInventory", "otherplayer", playerId)
end

Framework.GetCurrentWeapon = function() -- codem-inventory does not provide current weapon data 
    return false
end
