...
RegisterNetEvent('qb-pawnshop:client:openMelt', function(data)
    local inventory = lib.callback.await('qb-pawnshop:server:getPlayerInventory', false) or {}
    local meltMenu = {}
        ...
        for i = 1, #data.meltingItems do
            if invItem.name == data.meltingItems[i].item then
                meltMenu[#meltMenu + 1] = {
                    ...
                    args = {
                        name = invItem.name,
                        amount = invItem.amount or invItem.count,
                    }
                }
            end
        end
