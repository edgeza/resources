AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	if Config.Inv == "qb" and Core and Core.Shared and Core.Shared.Items then
	...
	elseif Config.Inv == "ox" then
		...
		if items then
			for k in pairs(Config.Prices) do if not items[k] then print("^5Debug^7: ^6Prices^7: ^2Missing Item from ox_inventory: '^6"..k.."^7'") end end
			if not items["recyclablematerial"] then print("^5Debug^7: ^2Missing Item from ox_inventory: '^6recyclablematerial^7'") end
			for _, v in pairs(Config.ScrapItems) do if not items[v] then print("^5Debug^7: ^6ScrapItems^7: ^2Missing Item from ox_inventory: '^6"..v.."^7'") end end
			for _, v in pairs(Config.DumpItems) do if not items[v] then print("^5Debug^7: ^2DumpItems^7: ^2Missing Item from ox_inventory: '^6"..v.."^7'") end end
		end
	end
end)
	...
	if give == 0 or give == false then
		if HasItem(src, item, amount or 1) then -- check if you still have the item
			...
			elseif Config.Inv == "qb" then
				...
				if Config.Debug and Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[item] then
					print("^5Debug^7: ^1Removing ^2from Player^7(^2"..src.."^7) '^6"..Core.Shared.Items[item].label.."^7(^2x^6"..(amount or "1").."^7)'")
				end
			else
				...
				if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[item] then
					TriggerClientEvent('codem-inventory:client:ItemBox', src, Core.Shared.Items[item], "remove", amount or 1)
				end
			end
		...
		elseif Config.Inv == "qb" then
			if Player.Functions.AddItem(item, amount or 1) then
				if Core and Core.Shared and Core.Shared.Items and Core.Shared.Items[item] then
					TriggerClientEvent('qb-iinventory:client:ItemBox', src, Core.Shared.Items[item], "add", amount or 1)
