...
Framework.OnReady(Framework, function()
    Framework.Items = {}
    -- Get items from jpr-inventory (same as qbx_core does)
    local items = exports['jpr-inventory']:GetItemList()
    if not items then
        -- Fallback: wait for jpr-inventory to be ready
        CreateThread(function()
            while not items do
                Wait(100)
                items = exports['jpr-inventory']:GetItemList()
            end
            -- Process items once they're available
...
Framework.AddItem = function(inventory, item, count, metadata, slot)
    ...
    elseif type(inventory) == "number" then
        ...
        if exports['jpr-inventory']:AddItem(inventory, item, count, metadata, slot) then
            TriggerClientEvent('inventory:client:ItemBox', inventory, Framework.Items[item], 'add', count)
            return true
        end
        return false
    end
...
Framework.RemoveItem = function(inventory, item, count, metadata, slot)
    ...
    elseif type(inventory) == "number" then
        ...
        if exports['jpr-inventory']:RemoveItem(inventory, item, count, metadata, slot) then
            TriggerClientEvent('inventory:client:ItemBox', inventory, Framework.Items[item], "remove", count)
            return true
        end
        return false
    end
...
Framework.GetItemCount = function(inventory, item, metadata, strict)
    ...
    elseif type(inventory) == "number" then
        local Player = Framework.GetPlayer(inventory)
        local items = exports['jpr-inventory']:GetItemsByName(inventory, item)
        for k, v in pairs(items) do
            if v.name ~= item then goto skipLoop end
...
Framework.GetItemMetadata = function(inventory, slot)
    ...
    elseif type(inventory) == "number" then
        local Player = Framework.GetPlayer(inventory)
        return exports['jpr-inventory']:GetItemBySlot(inventory, slot)?.info
    end
    return {}
end
...
Framework.SetItemMetadata = function(inventory, slot, metadata)
    ...
    elseif type(inventory) == "number" then
        local Player = Framework.GetPlayer(inventory)
        local item = exports['jpr-inventory']:GetItemBySlot(inventory, slot)
        if item then
            item.info = metadata
            exports['jpr-inventory']:SetItemData(inventory, slot, item)
        end
    end
end
...
Framework.ClearInventory = function(inventory, keep)
    ...
    elseif type(inventory) == "number" then
        local Player = Framework.GetPlayer(inventory)
        exports['jpr-inventory']:ClearInventory(inventory, keep)
    end
end
...
Framework.ConfiscateInventory = function(source)
    ...
    Player.Functions.SetMetaData("jailitems", exports['jpr-inventory']:GetInventory(src))
    Wait(2000)
    exports['jpr-inventory']:ClearInventory(src)
end
...
Framework.ReturnInventory = function(source)
    ...
    for _, item in pairs(jailitems) do
        if item then
            exports['jpr-inventory']:AddItem(src, item.name, item.amount, item.info)
            TriggerClientEvent('inventory:client:ItemBox', src, Framework.Items[item.name], 'add', item.amount)
        end
    end
    Wait(2000)
